...
exports.adminLogout = async (req, res) => {
  try {
    ...
    userText.activity.push(activityText._id);
    await userText.save();
    res.status(200).clearCookie("bearerToken").json({
      status: "success",
...
exports.getAllUsers = async (req, res) => {
  try {
    const userList = await User.find({ isAdmin: false, deletedAt: null })
      .select(
        "userName email emailVerified userStatus teamType role department access"
      )
      .populate({
        path: "department",
        select: "name",
...
exports.createUserPassword = async (req, res) => {
  try {
    ...
    const userText = await User.findByIdAndUpdate(
      { _id: _id },
      { password: pass2, userStatus: true },
      { new: true }
    );
    res.status(200).json({
      status: "success",
...
exports.changeUserStatus = async (req, res) => {
  try {
    ...
    const userTextStatus = await User.findById({ _id: userId }).select(
      "userStatus"
    );
    const userText = await User.findByIdAndUpdate(
      { _id: userId },
      { userStatus: !userTextStatus.userStatus },
      { new: true }
    );
    res.status(200).json({
      status: "success",
